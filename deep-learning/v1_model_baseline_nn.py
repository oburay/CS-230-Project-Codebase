...
class SCOTUSDeepLearning(nn.Module):
    """Neural network with learned embeddings for categorical features."""

    def __init__(self, vocab_sizes, embedding_dims, hidden_dims=[256, 128, 64], dropout=0.4):
        super().__init__()
        ...
        layers.extend([
            nn.Linear(total_embed_dim + 1, hidden_dims[0]),
...
def load_and_encode_data(path: Path = DATA_PATH):
    """Loads SCDB data and encodes categorical features."""
    df = pd.read_csv(path)
    ...
    numeric_data = df["term"].values

    # Conservative (1) -> 0, Liberal (2) -> 1
    y = (df[TARGET].values == 2).astype(int)

    return categorical_data, numeric_data, y, vocab_sizes, label_encoders, df["dateArgumentYear"].values


def calculate_embedding_dims(vocab_sizes):
    """Calculates embedding dimensions: min(50, (vocab_size + 1) // 2)"""
    return [min(50, (v + 1) // 2) for v in vocab_sizes]
...
def train_and_evaluate(
    ...
    categorical_data, numeric_data, y, vocab_sizes, label_encoders, years = load_and_encode_data()

    print(f"Total samples: {len(y)}")
    print(f"Features: {len(CATEGORICAL_FEATURES)} categorical + 1 numeric")
    print(f"Vocabulary sizes: {vocab_sizes}")
    print(f"Target distribution: {np.bincount(y)}\n")

    # Temporal split: train <=2019, test >=2020
    ...
    embedding_dims = calculate_embedding_dims(vocab_sizes)
    print(f"Embedding dimensions: {embedding_dims}\n")

    model = SCOTUSDeepLearning(
...
if __name__ == "__main__":
    ...
    model, encoders = train_and_evaluate()
